...
export default async function MonitorPage() {
    ...
    const [
        ...
        supabase.from('keywords').select('*', { count: 'exact', head: true }).not('total_doc_cnt', 'is', null),
        supabase.from('keywords').select('*', { count: 'exact', head: true }).eq('is_expanded', true),
        supabase.from('keywords').select('*', { count: 'exact', head: true }).eq('tier', 'PLATINUM'),
        supabase.from('keywords').select('*', { count: 'exact', head: true }).eq('tier', 'GOLD'),
        supabase.from('keywords').select('*').order('created_at', { ascending: false }).limit(10)
    ]);

    const total = totalCount || 0;
    const analyzed = analyzedCount || 0;
    const expanded = expandedCount || 0;
...
// Sub Components
function StatCard({ title, value, icon, desc, progress, color = "emerald" }: any) {
    return (
